...
- 语法诊断、补全、悬停、大纲、定义、参数提示，以及覆盖整个工作区的引用查询和重命名（局部变量、全局变量、可静态解析的表成员），支持 Lua 5.5 声明及本地模块。
- 解释器发现、切换和项目独立装包。
...
选中环境后，展开可查看标准库及函数，以及项目安装包和解释器搜索路径中的第三方模块。点击 Lua 模块可打开源码，原生模块可显示文件位置。F12 跳转和模块成员补全会使用当前解释器的 Lua 搜索路径、项目安装包，以及 `launch.json` 中 Lua 配置的 `packagePath`（支持 `${workspaceFolder}`、`${env:NAME}` 等目录级变量；相对路径按该配置的 `cwd` 解析，没有 `cwd` 时按工作区目录）；外部安装后点击刷新。扫描不执行第三方模块，也不反映运行时对 `package.path` 的修改。
