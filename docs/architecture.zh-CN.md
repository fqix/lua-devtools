...
`analysis/refactoring.go` 根据词法绑定查找引用并重命名局部符号，双向检查名称捕获。`analysis/references.go` 把两者扩展到全局变量和表成员：`ReferenceTargetAt` 判断光标下的名字属于哪一类，成员的身份是 *(所属表, 字段名)*——导出表用模块文件 URI 表示（因此可以跨文件比较），其他表用本次解析中的表指针。身份计算会不分作用域地回放文件里的全部成员写入，跟随别名、`__index` 链和 `require` 的模块，并记录每个字段的首次声明位置；而某个*名字*在某处绑定到哪张表则按位置、按源码顺序单独回放，名字在分支、循环或函数里被重新赋值而无法与该位置排序时，该位置标记为不可判定（引用查询仍按源码顺序的绑定列出，重命名则拒绝）。随后 `lsp/workspace.go` 扫描工作区根目录和已打开的文档（跳过隐藏目录和 `node_modules`，按文件数和大小设限，先按名字粗筛），对每个候选文件带模块解析地解析并收集匹配的位置；重命名为每个文件生成一个 `TextDocumentEdit`（打开的文档带版本号），拒绝会与已有成员、全局变量或捕获它的局部变量冲突的新名字，也拒绝声明文件不在扫描范围内的成员（已安装的包、工作区之外的文件），否则声明会被落下。LSP 返回带文档版本的编辑。`analysis/signature.go` 静态解析被调用函数和参数位置，并复用有界模块解析器提供跨模块函数声明。
