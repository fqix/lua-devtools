...
func TestInterpreterLanguageVersion(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		{`local value = 1`, true},
		{`local value =`, false},
		{`return 7 // 2`, runtime.version != "Lua 5.2"},
		{`return 3 & 1`, runtime.version != "Lua 5.2"},
		...
		if !ok || (len(diagnostics) == 0) != tt.valid {
			t.Errorf("%q: %+v, ok=%v", tt.source, diagnostics, ok)
		}
	}
	...
	if has("string", "pack") != (runtime.version != "Lua 5.2") {
	...
	for _, tt := range []struct {
		...
		{"string.", "pack", runtime.version != "Lua 5.2"},
		{"table.", "move", runtime.version != "Lua 5.2"},
		{"math.", "maxinteger", runtime.version != "Lua 5.2"},
	} {
...
func TestInterpreterDoesNotExecuteDocumentOrInit(t *testing.T) {
	...
	for _, version := range []string{"5_2", "5_3", "5_4", "5_5"} {
